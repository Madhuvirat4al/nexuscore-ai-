...
export default async function handler(req, res) {
  ...
  try {
    ...
    const gmailAppPassword = appPassword || process.env.GMAIL_APP_PASSWORD || process.env.GMAIL_PASS || '';

    console.log(`[Gmail SMTP Relay] Sending real-time OTP code ${otpCode} to ${email} via ${gmailUser}`);

    if (gmailAppPassword && gmailAppPassword.trim().length > 5) {
    ...
    // Default fallback if app password is not configured yet
    return res.status(200).json({
      success: true,
